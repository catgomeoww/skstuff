...
while (!feof($fail)){
    ...
    if($namajadual === "pelajar"){
        ...
        $idkelas = $medan[2];
        $password = $medan[3];
        $sql = "insert into pelajar value('$idpelajar', '$namapelajar', '$idkelas', '$password',)";
    ...
    if($namajadual === "soalan"){
        ...
        $jawapan = $medan[5];
        $idguru = $medan[6];
        $sql = "insert into soalan value('$idsoalan', '$namasoalan', '$pilihana', '$pilihanb', '$pilihanc', '$jawapan', '$idguru')";
        if (mysqli_query($sambungan, $sql))
            $berjaya = true;
        else
            $berjaya = false;
